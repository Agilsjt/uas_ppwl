...
<div class="container py-5">
    <div class="card border-0 rounded-4 shadow-lg" style="background-color: rgba(255, 255, 255, 0.08); backdrop-filter: blur(0);">
        <div class="card-body text-white">
            <h2 class="fw-bold mb-4">Edit Skill</h2>
            ...
            <form action="{{ route('skill.update', $skill->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control bg-white bg-opacity-75 text-dark border-0" id="name" name="name" required value="{{ old('name', $skill->name) }}">
                </div>

                <div class="mb-4">
                    <label for="description" class="form-label">Deskripsi</label>
                    <textarea class="form-control bg-white bg-opacity-75 text-dark border-0" id="description" name="description" rows="4">{{ old('description', $skill->description) }}</textarea>
                </div>
